...
Marketing site for **Afzal Dental** — turquoise/elegant theme, evening-appointment USP.

Built with [Astro 6](https://astro.build/) + [Tailwind CSS v4](https://tailwindcss.com/) (Vite plugin). Static output, deploys to Netlify.
...
1. Connect this repo in the Netlify dashboard (New site → Import from GitHub → `huzipatel/afzal-dental`).
2. Netlify auto-detects Astro and uses `netlify.toml` for build settings — no manual config needed.
3. Once the first build succeeds, add the custom domain `afzal.dental` under **Domain management** and follow the DNS instructions (CNAME on `www`, A or ALIAS on apex to Netlify's load balancer IPs).
